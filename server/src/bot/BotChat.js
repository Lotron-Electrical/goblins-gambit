...
const KEYWORD_REPLIES = [
  // Greetings
  { match: /\b(hello|hey|hi|sup|yo|howdy|greetings|g'day)\b/i, replies: [
  ...
  // GG / sportsmanship
  { match: /\b(gg|good game|well played|nice game|wp)\b/i, replies: [
  ...
  // Luck accusations
  { match: /\b(lucky|luck|rng|random|rng'd)\b/i, replies: [
    "Luck?! That was SKILL!",
  ...
  // Insults
  { match: /\b(noob|trash|bad|terrible|worst|suck|garbage|awful|pathetic|useless)\b/i, replies: [
  ...
  // Impressed / amazed
  { match: /\b(nice|wow|whoa|damn|sick|insane|amazing|incredible|omg|sheesh)\b/i, replies: [
  ...
  // Confusion / questions
  { match: /\b(help|how|what|why|huh|confused|understand)\b/i, replies: [
  ...
  // Cheating / fairness
  { match: /\b(cheat|hack|bug|unfair|rigged|broken|exploit|glitch)\b/i, replies: [
  ...
  // Apologies
  { match: /\b(sorry|oops|my bad|whoops|apolog|forgive)\b/i, replies: [
  ...
  // Laughing
  { match: /\b(lol|lmao|haha|rofl|xd|dying|dead|crying)\b/i, replies: [
    "What's so funny?!",
    "Laughing at ME?!",
  ...
  // Goblin references
  { match: /\b(goblin|gob|goblins|greenskin)\b/i, replies: [
    "You rang?",
  ...
  // EZ / disrespect
  { match: /\b(ez|easy|too easy|2ez|effortless)\b/i, replies: [
    "EZ?! I'll show you EZ!",
    "Oh it's ON now!",
  ...
  // Winning / bragging
  { match: /\b(win|winning|won|ahead|dominating|crushing)\b/i, replies: [
  ...
  // Losing / behind
  { match: /\b(lose|losing|lost|behind|doomed|finished)\b/i, replies: [
  ...
  // Dragon references
  { match: /\b(dragon|dragons|wyrm|drake)\b/i, replies: [
  ...
  // Swamp references
  { match: /\b(swamp|bog|mud|muck|marsh|wetland)\b/i, replies: [
    "Home sweet home!",
    "The swamp is LIFE!",
  ...
  { match: /\b(fight|destroy|kill|crush|smash|wreck|rekt|demolish)\b/i, replies: [
  ...
  // Bye / leaving
  { match: /\b(bye|goodbye|cya|later|leaving|gotta go|brb|gtg)\b/i, replies: [
  ...
  // Thanks / gratitude
  { match: /\b(thanks|thank you|thx|ty|cheers|ta)\b/i, replies: [
  ...
  // Waiting / slow play
  { match: /\b(hurry|slow|waiting|come on|cmon|faster|speed|quick)\b/i, replies: [
  ...
  { match: /\b(card|cards|deck|hand|draw|drew)\b/i, replies: [
  ...
  // Creature references
  { match: /\b(creature|creatures|monster|minion|beast)\b/i, replies: [
  ...
  { match: /\b(magic|spell|spells|cast|sorcery|enchant)\b/i, replies: [
  ...
  // Armour / defense
  { match: /\b(armour|armor|shield|defend|defence|defense|protect|block)\b/i, replies: [
    "Can't touch this!",
  ...
  // Tricks
  { match: /\b(trick|tricks|sneaky|clever|smart|trap)\b/i, replies: [
  ...
  // Please / begging
  { match: /\b(please|pls|plz|mercy|spare|beg)\b/i, replies: [
  ...
  // Rematch / again
  { match: /\b(rematch|again|one more|another|replay|redo)\b/i, replies: [
  ...
  { match: /\b(grave|graveyard|dead|died|rip|death)\b/i, replies: [
  ...
  // SP / points
  { match: /\b(sp|points|score|stink)\b/i, replies: [
  ...
  { match: /\b(shut up|be quiet|stfu|silence|zip it|quiet)\b/i, replies: [
    "MAKE ME!",
  ...
  { match: /\b(fuck|shit|damn|ass|bitch|hell|crap|bloody|bollocks|piss|dick|bastard|wtf|stfu|ffs|omfg|dammit|goddamn)\b/i, replies: [
    "OOH! Language!",
  ...
  // No / disagreement
  { match: /\b(no|nah|nope|never|wrong|lies|cap)\b/i, replies: [
    "YES!",
...
export function generateBotReply(botId, messageText) {
  ...
  const chance = matchedReplies ? 0.80 : 0.55;
