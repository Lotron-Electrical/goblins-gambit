...
async function register(username, password) {
  ...
  if (usePostgres) {
    const existing = await pool.query('SELECT 1 FROM gg_accounts WHERE username = $1', [key]);
...
async function login(username, password) {
  ...
  if (usePostgres) {
    ...
    if (result.rows.length === 0) return { error: 'Invalid username or password' };
    ...
    if (usePostgres) {
      ...
      if (Object.keys(gameCardCounts).length > 0) {
        ...
        for (const [name, count] of Object.entries(gameCardCounts)) {
          account.stats._cardCounts[name] = (account.stats._cardCounts[name] || 0) + count;
        }
        let maxCount = 0;
...
export { initDatabase, register, login, getProfile, getLeaderboard, validateToken, updateStatsAfterGame };
