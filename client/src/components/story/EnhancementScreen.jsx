...
export default function EnhancementScreen() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-gradient-to-b from-gray-950 via-green-950/20 to-gray-950">
      <h2 className="text-3xl font-display text-green-400 mb-2">Choose an Enhancement</h2>
      <p className="text-gray-400 text-sm mb-8">Pick one upgrade for your creature</p>
        ...
        {enhancementOptions.map((opt, idx) => (
          ...
          >
            <span className="text-3xl">
              {opt.type === "stat_boost" ? "\uD83D\uDCAA" : opt.type === "ability" ? "\u2728" : opt.type === "life" ? "\u2764\uFE0F" : opt.type === "item" ? "\uD83C\uDF92" : "\u2B50"}
            </span>
            <span className="text-white font-bold text-sm">{opt.description}</span>
            <span className="text-gray-500 text-xs">
              {opt.type === "stat_boost" && `+${opt.amount} ${opt.stat?.toUpperCase()}`}
      ...
      {storyRun && (
        <div className="mt-6 text-xs text-gray-500">
          Current stats: {storyRun.customCard.attack}/{storyRun.customCard.defence}/{storyRun.customCard.sp}
          {storyRun.customCard.abilityId && ` | Ability: ${storyRun.customCard.abilityId}`}
        </div>
      )}
    </div>
  );
