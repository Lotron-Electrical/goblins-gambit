...
function MobileCardInfoPanel({
  ...
  const isTutorialBlocked =
    tutorialEngine &&
    tutStepConfig?.highlightCardUid &&
    tutStepConfig.highlightCardUid !== card.uid;
  const isTutorialHighlighted =
    tutorialEngine && tutStepConfig?.highlightCardUid === card.uid;
    ...
    >
      <div className="flex gap-2 p-2">
        ...
        <div className="flex-1 min-w-0 space-y-1">
          <div>
            <h3 className="font-display text-[14px] text-white leading-tight truncate">
              {card.name}
            </h3>
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              {card.cost !== undefined && (
                <span
                  className={`text-[11px] ml-auto font-bold ${effectiveCost === 0 ? "text-green-400" : costModified ? "text-red-400" : "text-blue-300"}`}
                >
          ...
          {confirmAction === "discard" ? (
            <div className="bg-red-950 border border-red-700 rounded-lg p-2 space-y-1.5">
              <p className="text-[11px] text-red-200 text-center">
                Discard {card.name}? Cannot be undone.
              </p>
              <div className="flex gap-2">
                <button
                  onClick={() => setConfirmAction(null)}
                  className="flex-1 bg-gray-700 text-white font-bold py-1.5 rounded text-[11px]"
                >
                  Cancel
                </button>
                <button
                  onClick={() => discardCard(card.uid)}
                  className="flex-1 bg-red-700 text-white font-bold py-1.5 rounded text-[11px]"
                >
                  Confirm
                </button>
              </div>
...
function CircularCardRow({
  ...
  const handleTouchMove = useCallback(
    (e) => {
      ...
      const clamped = Math.max(
        -0.4,
        Math.min(newIndex, sortedHand.length - 0.6),
      );
        ...
        rafId.current = requestAnimationFrame(() => {
          ...
          if (pendingIndex.current !== null) {
            ...
            const rounded = Math.round(
              Math.max(
                0,
                Math.min(pendingIndex.current, sortedHand.length - 1),
              ),
            );
  ...
  const handleTouchEnd = useCallback(() => {
    ...
    const runInertia = () => {
      ...
      if (Math.abs(vel) > VELOCITY_THRESHOLD) {
        ...
        const target = Math.round(
          Math.max(0, Math.min(currentIndexRef.current, maxIdx)),
        );
        const easeToTarget = () => {
          const cur = currentIndexRef.current;
          const diff = target - cur;
    ...
    if (Math.abs(vel) > VELOCITY_THRESHOLD) {
      ...
      const target = Math.round(
        Math.max(0, Math.min(currentIndexRef.current, maxIdx2)),
      );
      const easeToTarget = () => {
        const cur = currentIndexRef.current;
        const diff = target - cur;
  ...
  if (sortedHand.length === 0) {
    return (
      <div className="text-gray-600 py-4 text-[11px] w-full text-center">
        No cards in hand
      </div>
    );
  }
  ...
  return (
    <div className="relative" style={{ height: "150px", overflow: "hidden" }}>
      <div
        ...
        onTouchStart={
          scrollLocked ? undefined : reorderMode ? undefined : handleTouchStart
        }
        onTouchMove={
          scrollLocked
            ? undefined
            : reorderMode
              ? handleReorderMove || handleTouchMove
              : handleTouchMove
        }
        onTouchEnd={
          scrollLocked
            ? undefined
            : reorderMode
              ? handleReorderEnd || handleTouchEnd
              : handleTouchEnd
        }
        onTouchCancel={
          scrollLocked
        ...
        {sortedHand.map((card, idx) => {
          ...
          return (
            ...
            >
              <div
                onTouchStart={
                  reorderMode ? () => handleReorderStart(idx) : undefined
                }
              >
                <CardInHand
                  card={card}
                  ...
                  variant="row"
                  onSelect={
                    reorderMode ? undefined : (c) => handleCardTap(c, idx)
                  }
...
export default function HandBar() {
  const { gameState, selectedCard, selectCard, drawCard, endTurn, buyAP } =
    useStore();
  ...
  const hasReaction =
    !isMyTurn && hand.some((c) => REACTION_ABILITIES.includes(c.abilityId));
  ...
  const tutorialHighlightUid = tutorialEngine
    ? tutorialEngine.getStepConfig()?.highlightCardUid
    : null;
  const tutorialTabHint = tutorialEngine
    ? tutorialEngine.getStepConfig()?.tabHint
    : null;
  ...
  const sortedHand = useMemo(() => {
    if (sortMode === "type") {
      return [...hand].sort(
        (a, b) =>
          ...
          getEffectiveCost(a) - getEffectiveCost(b),
      );
    }
    if (sortMode === "cost") {
      return [...hand].sort(
        (a, b) => getEffectiveCost(a) - getEffectiveCost(b),
      );
    }
    ...
    if (customOrder) {
      ...
      return [...hand].sort(
        (a, b) => (orderMap[a.uid] ?? 999) - (orderMap[b.uid] ?? 999),
      );
    }
    return hand;
  ...
  useEffect(() => {
    if (handExpanded && hand.length > prevHandCountExpanded.current) {
      ...
      const timer = setTimeout(
        () => setCarouselScrollTo(sortedHand.length - 1),
        50,
      );
  ...
  const cycleSortMode = useCallback(() => {
    setSortMode((prev) =>
      prev === null ? "type" : prev === "type" ? "cost" : null,
    );
        ...
        <AnimatePresence>
          {handExpanded && (
            <motion.div
              ...
              onClick={() => {
                soundManager.play("hand_whoosh_close");
        ...
        {!handExpanded && (
          <div
            className={`bg-gray-950/90 border-t border-gray-800 px-2 py-1 ${
              stripFlash
                ? "ring-2 ring-[var(--color-gold)] animate-[pulse_0.3s_ease-in-out_2]"
                : ""
            }`}
            onClick={() => {
              if (hand.length > 0) {
          ...
          >
            ...
            <div className="flex items-center justify-between mb-1">
              <span className="text-gray-500 text-[10px]">
                {hand.length} card{hand.length !== 1 ? "s" : ""} — tap to view
              </span>
