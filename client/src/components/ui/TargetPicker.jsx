...
export default function TargetPicker({ mobileCenterY }) {
  ...
  if (!pending.validTargets || pending.validTargets.length === 0) {
    return (
      <div className="fixed inset-0 z-40 bg-black/50 pointer-events-auto flex items-center justify-center">
        <div
          className={`bg-gray-900 border border-[var(--color-gold)] rounded-xl shadow-2xl ${isMobile ? "p-3 max-w-sm" : "p-6 max-w-md"}`}
        >
          <h3
            className={`font-display text-[var(--color-gold)] ${isMobile ? "text-lg mb-2" : "text-xl mb-3"}`}
          >
            No Valid Targets
          </h3>
          <p
            className={`text-gray-300 ${isMobile ? "text-[13px] mb-3" : "text-[14px] mb-4"}`}
          >
            {pending.prompt}
          </p>
          <button
  ...
  const handleConfirmMulti = () => {
    ...
    selectTarget(multiSelected[0].ownerId, multiSelected[0].uid, targets);
    setMultiSelected([]);
  };
  ...
  return (
    <div className="fixed inset-0 z-40 bg-black/50 pointer-events-auto">
      ...
      >
        <div
          className={`bg-gray-900 border border-[var(--color-gold)] rounded-xl shadow-2xl w-full ${
        ...
        >
          ...
          <div
            className={`space-y-2 overflow-y-auto ${isMobile ? "max-h-[250px]" : "max-h-[300px]"}`}
          >
            {pending.validTargets.map((target) => {
              const isMultiSel = multiSelected.some(
                (t) => t.uid === target.uid,
              );
              const ownerName =
                gameState.players[target.ownerId]?.name || "Unknown";
              const possessive =
              ...
              return (
                ...
                >
                  <span
                    className={`text-white font-medium ${isMobile ? "text-[13px]" : "text-[14px]"}`}
                  >
                    {target.name}
                  </span>
                  <span
                    className={`text-gray-400 ${isMobile ? "text-[11px]" : "text-[12px]"}`}
                  >
                    {isPlayerTarget
