...
export default function ChatPanel({ expanded, onClose }) {
  ...
  const handleSend = () => {
    ...
    if (!trimmed) return;
    if (isTutorial) return; // No sending during tutorial
    if (!trySend()) return;
  ...
  return (
    <>
      {/* Invisible backdrop — tap outside to close (disabled during tutorial) */}
      {!isTutorial && (
        <div className="fixed inset-0 z-[39]" onClick={onClose} />
      )}
      <div
        className={`fixed right-2 z-40 bg-gray-950/95 border rounded-lg shadow-2xl overflow-hidden flex flex-col ${
      ...
      >
        ...
        <div className="flex items-center justify-between px-2 py-1 border-b border-gray-800 shrink-0">
          <span
            className={`text-[11px] font-bold uppercase tracking-wider ${
              isTutorial ? "text-[var(--color-gold)]" : "text-gray-400"
            }`}
          >
            {isTutorial ? "Gnarl's Guide" : "Chat"}
          </span>
          <div className="flex items-center gap-2">
            {isTutorial && (
              <button
                onClick={() => endTutorial()}
                className="text-gray-500 hover:text-gray-300 text-[10px] px-1.5 py-0.5 bg-gray-800 border border-gray-700 rounded transition"
              >
                Skip
              </button>
            )}
        ...
        >
          ...
          {chatMessages.map((msg) => {
            ...
            const isGnarl = msg.isGnarl;
            const emote = msg.emoteKey ? emoteMap[msg.emoteKey] : null;
            const nameColor = isGnarl
              ? "#d4af37"
              : isOwn
                ? "#9ca3af"
                : colorForPlayer(msg.playerId);
                ...
                >
                  <div className="text-[12px] leading-snug break-words">
                    ...
                    <span
                      className={
                        msg.isThinking
                          ? "text-gray-400 italic animate-pulse"
                          : "text-gray-200"
                      }
                    >
                      {msg.text}
        ...
        {/* Emote row — hidden during tutorial */}
        {!isTutorial && (
        ...
        {/* Text input — hidden during tutorial */}
        {!isTutorial && (
