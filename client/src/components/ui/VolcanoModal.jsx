    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-orange-400 font-display text-xl">&#x1F30B; Volcano Bank</h3>
          <button onClick={onClose} className="text-gray-500 hover:text-white text-lg">x</button>
        </div>

        <div className="text-gray-300 text-sm mb-3">
          Deposit SP for interest. Roll D6 = lock duration + interest rate (D6 x 10%).
          <span className="text-red-400 block mt-1">Warning: If a Dragon appears, deposits are frozen and the slayer takes everything!</span>
        </div>

        <div className="bg-gray-800 rounded-lg p-3 mb-3">
          <div className="flex justify-between text-sm">
            <span className="text-gray-400">Total Banked</span>
            <span className="text-orange-400 font-bold">{volcano.totalBanked} SP</span>
          </div>
        ...
        {myDeposits.length > 0 && (
          <div className="mb-3">
            <h4 className="text-gray-400 text-xs uppercase mb-1">Your Deposits</h4>
            <div className="space-y-1">
              {myDeposits.map((dep, i) => (
                <div key={i} className="flex justify-between text-xs bg-gray-800/60 rounded px-2 py-1">
                  <span className="text-orange-300">{dep.amount} SP @ {dep.interestRate}%</span>
                  <span className="text-gray-400">{dep.maturesIn} cycles left</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={!amount || parseInt(amount) <= 0 || parseInt(amount) > (myPlayer?.sp || 0)}
        ...
        {!isMyTurn && (
          <div className="text-gray-500 text-sm text-center">Wait for your turn to deposit</div>
        )}
      </div>
    </div>
  );
