...
function formatEvent(evt, players) {
  ...
  switch (evt.type) {
    ...
    case "attack":
      return evt.directAttack
        ? `Direct attack on ${name(evt.defenderOwner)}!`
        : `Attack!`;
    case "damage":
    ...
    case "sp_change":
      if (evt.amount > 0)
        return `${name(evt.playerId)} gained ${evt.amount} SP${evt.reason ? ` (${evt.reason})` : ""}`;
...
export default function ActivityLog() {
  ...
  const getColor = (type) => {
    switch (type) {
      ...
      case "game_over":
        return "text-[var(--color-gold)] font-bold";
      default:
      ...
      {!collapsed && (
        ...
        >
          ...
          {entries.map((entry) => (
            <div
              key={entry.id}
              className={`text-[11px] leading-tight py-0.5 ${getColor(entry.type)}`}
            >
              {entry.text}
            </div>
          ))}
