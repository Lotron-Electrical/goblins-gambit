...
import {
  ...
  getBotName,
  decideBotAction,
} from "./server/src/bot/BotPlayer.js";
...
function runGame(gameNumber) {
    ...
    if (action.type === ACTION.ATTACK && result.success) {
      for (const id of botIds) {
        const currentUids = new Set(
          engine.state.players[id].swamp.map((c) => c.uid),
        );
        ...
        for (const uid of preUids) {
          if (!currentUids.has(uid)) {
            ...
            const me = engine.state.players[currentId];
            const attackerCard = me?.swamp.find(
              (c) => c.uid === action.attackerUid,
            );
            if (attackerCard) {
              creatureKills[attackerCard.name] =
                (creatureKills[attackerCard.name] || 0) + 1;
            }
            ...
            if (placed) {
              ...
              if (!creatureTurnsAlive[placed.name])
                creatureTurnsAlive[placed.name] = [];
  ...
  return {
    ...
    creatureTurnsAlive,
    spHistory,
    finalSP: Object.fromEntries(
      botIds.map((id) => [
        engine.state.players[id].name,
        engine.state.players[id].sp,
      ]),
    ),
...
for (let i = 1; i <= NUM_GAMES; i++) {
  ...
  console.log(
    `Game ${i}: ${status} | Turn ${result.turnNumber} | Winner: ${winnerStr}`,
  );

  // Print final SP
  const spStr = Object.entries(result.finalSP)
    .map(([n, sp]) => `${n}: ${sp}`)
    .join(" | ");
...
for (let i = 0; i < 4; i++) {
  ...
  console.log(
    `  Position ${i + 1}: ${winsByPosition[i]}/${NUM_GAMES} wins (${pct}%)`,
  );
}
...
// Most played cards
const sortedPlayed = Object.entries(aggregateCardsPlayed).sort(
  (a, b) => b[1] - a[1],
);
console.log();
